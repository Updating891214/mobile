...
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','send_pwd_sms','password', 'signin', 'add_tag', 
    ...
    'get_passwd_question', 'check_answer', 'oath', 'oath_login','send_pwd_phonecode');
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $username = isset($_POST['mobile_phone']) ? trim($_POST['mobile_phone']) : '';
 ...
 */
         if (register($username, $password, $other) !== false)
        {
            ...
            $db->query("update ".$ecs->table("users")." set mobile_phone='".$mobile_phone."' where user_id=".$_SESSION['user_id']."");
            /*把新注册用户的扩展信息插入数据库*/
