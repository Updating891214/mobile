...
elseif($act=='update_group_cart')
{
	...
	if(!$goods && $rec_id==$goods_id){
		$sql = "SELECT  `rec_id`,`goods_id`, `goods_attr_id`, `product_id`, `extension_code` FROM" .$GLOBALS['ecs']->table('cart').
		   " WHERE goods_id='$rec_id' AND session_id='" . SESS_ID . "'";
		$goods = $GLOBALS['db']->getRow($sql);
		if($goods){
			$rec_id=$goods['rec_id'];
		}
