...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        elseif (!empty($image_urls[$key]) && ($image_urls[$key] != $GLOBALS['_LANG']['img_file']) && ($image_urls[$key] != 'http://') && copy(trim($image_urls[$key]), ROOT_PATH . 'data/' . basename($image_urls[$key])))
        {
            $image_url = trim($image_urls[$key]);

            //定义原图路径
            $down_img = ROOT_PATH . 'data/' . basename($image_url);
