    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('touch_nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type')";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('touch_nav') . " (name,ifshow,vieworder,opennew,url,type) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type')";
        }
        $db->query($sql);
        clear_cache_files();
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('touch_nav') .
                " SET name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',pic='$item_pic',type='$item_type' WHERE id='$id'";
        }
        else
        {
            //目标不是分类
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('touch_nav') .
                " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',pic='$item_pic',type='$item_type' WHERE id='$id'";
        }
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT id, name, ifshow, vieworder, opennew, url, pic, type".
    ...
    foreach($navdb as $k=>$v)
    {
        ...
        $navdb2[$k] = $v;
        $navdb2[$k]['pic'] = '../themes/' . $_CFG['template'] . '/images/'.(empty($v['pic'])?'ico.png':$v['pic']);
        $type = $v['type'];
    }
