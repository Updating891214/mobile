...
elseif($_REQUEST['act'] == 'install' || $_REQUEST['act'] == 'view')
{
	...
	$filepath = ROOT_PATH . 'include/website/';
...
elseif($_REQUEST['act']  == 'query_install' || $_REQUEST['act'] == 'update_website')
{
	...
	$commnet .=	'?>';
	$filename = ROOT_PATH . 'include/website/config/'.$type.'_config.php';
...
function getWebsiteList()
{
	...
	{
		if($file != '.' && $file != '..' && $file != 'jntoo.php' && $file != 'config' && substr($file , 0 , 3) != 'cls' && substr($file , 0 , 4) != 'oath')
		{
